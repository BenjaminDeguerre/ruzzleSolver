...
void test_2_LinkedList(void)
{
  ...
  LL_add(&maList, (void *)wordTest, W_copy);
...
void test_5_LinkedList(void)
{
  ...
  W_Word *wordTest = W_createWord("plop");
  LL_add(&maList, (void *)wordTest, W_copy);
  LL_add(&maList1, (void *)wordTest, W_copy);
...
{
  return (
      (NULL == CU_add_test(pSuite, "LL_isEmpty(LL_createListOrdonne() == 1",
                           test_1_LinkedList)) ||
      (NULL == CU_add_test(pSuite, "!LL_isEmpty(LL_add(l,e,e_copy)) == 1",
                           test_2_LinkedList)) ||
      (NULL == CU_add_test(pSuite, "LL_getElement(LL_add(l,e,e_copy)) == e ",
                           test_3_LinkedList)) ||
      (NULL == CU_add_test(pSuite, "LL_getNextList(LL_add(l,e,e_copy)) == l",
                           test_4_LinkedList)) ||
      (NULL == CU_add_test(pSuite, "LL_equals(l,l) == 1", test_5_LinkedList)) ||
