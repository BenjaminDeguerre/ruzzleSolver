...
int test_Node(CU_pSuite pSuite) {
  return (
      (NULL ==
       CU_add_test(pSuite,
                   "initialisation d'un node, value == plop, isLast == 1",
                   test_1_Node)) ||
      (NULL == CU_add_test(pSuite, "test fonction N_getValue", test_2_Node)) ||
      (NULL == CU_add_test(pSuite, "test fonction N_isLast", test_3_Node)) ||
      (NULL ==
       CU_add_test(pSuite, "test fonction N_defineValue", test_4_Node)) ||
      (NULL == CU_add_test(pSuite, "test fonction N_defineEnd", test_5_Node)) ||
      (NULL ==
       CU_add_test(pSuite, "test fonction N_compare (vrai)", test_6_Node)) ||
      (NULL ==
       CU_add_test(pSuite, "test fonction N_compare (faux)", test_7_Node)));
}
