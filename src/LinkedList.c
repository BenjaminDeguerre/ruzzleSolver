...
{
	errno = 0;
	return list->nextLinkedList == NULL;
}

int LL_isEmpty(LL_LinkedList list)
{
	errno = 0;
	return (list->element == NULL) && (list->nextLinkedList == NULL);
}
...
int LL_equals(LL_LinkedList list1, LL_LinkedList list2, ES_compare compareElement)
{
	...
	else
	{
		...
		else
		{
			...
			{
				return LL_equals(LL_getNextList(list1), LL_getNextList(list2), compareElement);
			}
			else
			{
				return 0;
