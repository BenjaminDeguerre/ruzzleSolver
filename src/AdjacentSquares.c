...
{
	errno = 0;
	AS_AdjacentSquares *adjacentSquare = (AS_AdjacentSquares *)malloc(sizeof(AS_AdjacentSquares));
...
void AS_addSquare(AS_AdjacentSquares *adjacentSquare, S_SQUARE *square)
{
	...
	{
		AS_setNbPoints(adjacentSquare, AS_getNumberOfPoints(*adjacentSquare) + (S_getPointsNumber(*square) - 48) * vBonusLetter);
	}
	free(Bonus);
}
...
{
	char *squaresString = (char *)malloc(sizeof(char) * AS_getLength(adjacentSquare));
