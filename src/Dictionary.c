...
int wordIsINR(char *stringToTest, BT_BinaryTree *tree)
{
  ...
  {
    nextStringToTest = subString(stringToTest, 1, strlen(stringToTest) - 1);
    stringToTest = stringToTest[0];
    adjacentCurrentTree = N_getValue(*BT_getRoot(*tree));
    ...
    else if (stringToTest == adjacentCurrentTree)
    {
      ...
      if (!BT_isEmpty(tmp))
      {
        res = (wordIsINR(nextStringToTest, &tmp));
      }
      else
      {
        res = -1;
...
void unserializeTree(FILE *file, BT_BinaryTree *tree)
{
  ...
  if (fgets(line, LINE_LENGTH, file) != NULL)
  {
    if ((line[0] >= 97) && (line[0] <= 122))
    {
      ...
      else if (!BT_isEmpty(*tree) && BT_isEmpty(BT_getLeftChild(*tree)) && !BT_isProcessed(tree))
      {
        N_Node *N2 = N_createNode(0, line[0]);
        BT_setLeftChild(tree, BT_addRoot(BT_createSBT(), BT_createSBT(), N2, (ES_copy)N_copy));
        N_free(N2);
          ...
          if (!BT_isEmpty(*tree) && BT_isEmpty(BT_getRightChild(*tree)))
      {
        ...
        BT_setRightChild(tree, BT_addRoot(BT_createSBT(), BT_createSBT(), N3, (ES_copy)N_copy));
        N_free(N3);
