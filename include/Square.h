...
/** 
 *	\typedef S_SQUARE
 *	\brief Structure Square, contient une chaine de caractère ainsi que ça position dans la grille 
 */
typedef struct {
	char *value;	/**< chaine de caractère contenant la lettre, la value de la case et le bonus associé (dans cet ordre !) */
 ...
 *	\brief retourne la lettre contenue dans la case (première value de la chaine de caractère) 
 *	\param c la case
 *	\return char
 */
...
/** 
 *	\fn S_SQUARE S_defineSquare(S_SQUARE square, char *value, int noSquare)
 *	\brief permet de remplir la case avec les bonnes values 
 *	\param c la case
 *	\param *value, chaine de caractère contenant la lettre, la value de la case et le bonus associé
 *	\param noSquare le numéro de la case
 *	\return void
 */
 ...
 *	
 *	Deux cases sont égales quand elles ont la même value, ced même chaine de caractère 
