 ...
 *
 *	Cette structure contient, une liste ordonnée de case, le nombre de
 *points des cases (sans le bonus multiplicateur sur le mot mais avec celui sur
 *la lettre, et le coefficient du bonus sur le mot
 */
typedef struct {
  LL_LinkedList linkedList;
  int numberOfPoints;   /**< nombre de point des cases (somme), sans compter mot
                           compte double, triple... */
  int coefficientBonus; /**< contient le coefficient multiplicateur du bonus sur
                           le mot */
} AS_AdjacentSquares;
 ...
 *	\brief Créé la suite de case, linkedList vide et numberOfPoints,
 *coefficientBonus à 0
 *	\return AS_AdjacentSquares*
 */
AS_AdjacentSquares *AS_createAdjacentSquares();

/**
 *	\fn LL_LinkedList AS_getPossibleNeighbours(G_Grid *grid,
 *AS_AdjacentSquares *adjacentSquare)
 *	\brief Permet d'get tout les cases neighbours possible de la dernière
 *case
 *
...
/**
 *	\fn void AS_addSquare(AS_AdjacentSquares *adjacentSquare, S_SQUARE
 **square)
 *	\brief ajoute une case en tête de string et met à jour les values de
 *point bonus et du nombre de points
 *	\param *adjacentSquare Les cases contigues auxquelle on veut radd la
 *case (pointeur)
